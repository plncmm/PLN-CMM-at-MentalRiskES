...
post_data_per_round = {"task1a": {}, "task1b":{}}
finished_serving = False
completed_runs_task1a = set()
completed_runs_task1b = set()

user_path = os.path.join("data","task1","trial","subjects_trial")

def read_json_file(filename):
    with open(filename,mode="r", encoding="utf-8-sig") as json_file:
        return json.load(json_file)

@app.route('/task1/getmessages_trial/789c16fe57f3bc7f0108525b1ea89a0bb4a4952b', methods=['GET'])
def serve_messages():
    global current_round, waiting_for_post, finished_serving, completed_runs_task1a, completed_runs_task1b


    if waiting_for_post and len(completed_runs_task1a) < NUMBER_OF_RUNS:
        lacking = NUMBER_OF_RUNS - len(completed_runs_task1a)
        return make_response(jsonify({"error": f"Lacking {lacking} POST requests for task1a"}), 400)  
    
    if waiting_for_post and len(completed_runs_task1b) < NUMBER_OF_RUNS:
        lacking = NUMBER_OF_RUNS - len(completed_runs_task1b)
        return make_response(jsonify({"error": f"Lacking {lacking} POST requests for task1a"}), 400) 
    ...
    if not messages:
        ...
        print("No more messages, saving data to predictions_task1.json")
        with open('predictions_task1.json', 'w') as f:
...
@app.route('/task1a/submit_trial/789c16fe57f3bc7f0108525b1ea89a0bb4a4952b/<int:runs>', methods=['POST'])
def process_data_task1a(runs):
    global waiting_for_post, post_data_per_round, current_round, completed_runs_task1a, completed_runs_task1b
    ...
    post_data_per_round["task1a"].setdefault(current_round - 1, {})[runs] = data
    completed_runs_task1a.add(runs)

    if len(completed_runs_task1a) == NUMBER_OF_RUNS and len(completed_runs_task1b) == NUMBER_OF_RUNS:
        waiting_for_post = False
        completed_runs_task1a.clear()
        completed_runs_task1b.clear()
...
@app.route('/task1b/submit_trial/789c16fe57f3bc7f0108525b1ea89a0bb4a4952b/<int:runs>', methods=['POST'])
def process_data_task1b(runs):
    global waiting_for_post, post_data_per_round, current_round, completed_runs_task1a, completed_runs_task1b
    ...
    post_data_per_round["task1b"].setdefault(current_round - 1, {})[runs] = data
    completed_runs_task1b.add(runs)
