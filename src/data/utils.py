...
def download_messages_train(task: str, subtasks: List[str], token: str) -> List[Dict]:
    response = requests.get(
        ENDPOINT_DOWNLOAD_MESSAGES_TRAIN.format(TASK=task, TOKEN=token)
    )
    ...
    for subtask in subtasks:
        response = requests.get(
            ENDPOINT_DOWNLOAD_GOLD_TRAIN.format(SUBTASK=subtask, TOKEN=token)
        )
        ...
        if response.status_code != 200:
            ...
            file_object = open(
                "../../data/raw/{task}/train/gold_train_{subtask}.txt".format(
...
def download_data(task):
    ...
    if task == 2:
        download_messages_train(
            "task2", ["task2a", "task2b", "task2c", "task2d"], TOKEN
        )
    if task == 3:
